package com.hutoslib.client.particle.util;
...
public class ParticleUtils {
	...
	public static void beam(BlockPos toThisBlock, BlockPos fromThisBlock, Level world) {
		...
		while ((d4 + .65) < d3) {
			...
			if (world instanceof ServerLevel) {
				((ServerLevel) world).sendParticles(ParticleTypes.WITCH, x1 + d0 * d4, y1 + d1 * d4, z1 + d2 * d4,
	...
	public static void spawnPoof(ServerLevel world, BlockPos pos) {
		for (int i = 0; i < 10; i++) {
			...
			(world).sendParticles(ParticleTypes.END_ROD, d0, d1, d2, 2, (world.random.nextFloat() * 1 - 0.5) / 3,
