...
public abstract class GuiGuideTitlePage extends Screen {
	...
	public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		if (!title.getContents().isEmpty()) {
			GuiUtils.drawMaxWidthString(font, title, centerX + 10, centerY + 10, 165, 0xffffff, true);
		}
	...
	public void init() {
		...
		for (int i = 0; i < categories.size(); i++) {
			buttonList.add(new GuiButtonTextured(LocationHelper.guiPrefix("book_tabs.png"), i,
					(int) (centerX + (guiWidth * 0.05f) + 167 + (rand.nextInt(6) - rand.nextInt(4))),
					(int) (centerY - (i * -25) + 18), 24, 16, 0, 32 * i,
					new TextComponent(TextUtils.toProperCase(categories.get(i).category.toString())), (press) -> {
