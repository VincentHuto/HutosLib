...
public abstract class GuiGuidePage extends Screen {
	...
	static Component titleComponent =  new TranslatableComponent("");
	...
	public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		if (pageNum != 0) {
			HLGuiUtils.drawMaxWidthString(font,  new TranslatableComponent("Pg." + pageNum), left + guiWidth - 26,
		...
		matrixStack.popPose();
		if (!title.isEmpty()) {
			HLGuiUtils.drawMaxWidthString(font,  new TranslatableComponent(I18n.get(title)), left - guiWidth + 180,
					top + guiHeight - 220, 165, 0xffffff, true);
		}
		if (!subtitle.isEmpty()) {
			HLGuiUtils.drawMaxWidthString(font,  new TranslatableComponent(I18n.get(subtitle)), left - guiWidth + 180,
					top + guiHeight - 210, 165, 0xffffff, true);
		}

		if (!text.isEmpty() && subtitle.isEmpty() && title.isEmpty()) {
			HLGuiUtils.drawMaxWidthString(font,  new TranslatableComponent(I18n.get(text)), left - guiWidth + 180,
		...
		if ((mouseX >= left + guiWidth - 32 && mouseX <= left + guiWidth - 10)) {
			if (mouseY >= top + guiHeight - 220 && mouseY <= top + guiHeight - 200) {
				...
				if (!icon.isEmpty()) {
					text.add( new TranslatableComponent(I18n.get(icon.getHoverName().getString())));
...
//		titlePage.add( new TranslatableComponent(I18n.get("Title")));
//		titlePage.add( new TranslatableComponent(I18n.get("Return to Catagories")));
...
//		}
		...
		ClosePage.add( new TranslatableComponent(I18n.get("Close Book")));
...
//				}));
		...
		textBox = new EditBox(font, left - guiWidth + 155, top + guiHeight - 227, 14, 14,  new TranslatableComponent(""));
		super.init();
	}
