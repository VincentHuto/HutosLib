...
public abstract class GuiGuidePage extends Screen {
	...
	public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		if (pageNum != 0) {
			HLGuiUtils.drawMaxWidthString(font, new TextComponent("Pg." + pageNum), left + guiWidth - 26,
		...
		matrixStack.popPose();
		if (!title.isEmpty()) {
			HLGuiUtils.drawMaxWidthString(font, new TextComponent(I18n.get(title)), left - guiWidth + 180,
					top + guiHeight - 220, 165, 0xffffff, true);
		}
		if (!subtitle.isEmpty()) {
			HLGuiUtils.drawMaxWidthString(font, new TextComponent(I18n.get(subtitle)), left - guiWidth + 180,
					top + guiHeight - 210, 165, 0xffffff, true);
		}

		if (!text.isEmpty() && subtitle.isEmpty() && title.isEmpty()) {
			HLGuiUtils.drawMaxWidthString(font, new TextComponent(I18n.get(text)), left - guiWidth + 180,
